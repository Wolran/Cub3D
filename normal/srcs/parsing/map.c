...
/*   Updated: 2023/11/02 17:44:38 by vmuller          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static inline int	__line_get_map(char *str, t_pars *pars)
{
	...
	while (*ptr && ft_strchr(" 01NOWE", *ptr))
	{
		if (*ptr && ft_strchr("NOWE", *ptr))
		{
			...
			pars->rotation = *ptr;
			pars->spawn = (t_v2i){ptr - str, pars->data.size};
		}
		ptr++;
	}
	if (!ft_strchr(" 01NOWE\n\0", *ptr))
...
int	pars_map(int const fd, t_pars *const pars)
{
	...
	pars->size = (t_v2i){pars->size[x], pars->data.size};
	if (pars->size[x] < 3 || pars->size[y] < 3)
